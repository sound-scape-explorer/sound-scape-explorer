...
#         # Decoder
    ...
    def encode(self, x):
        ...
        for i in range(len(self.HiddenLayersIn)):
            linLast = self.relu(self.HiddenLayersIn[i][1]\
    ...
    def decode(self, z):
        ...
        for i in range(len(self.HiddenLayersOut)):
            linLast = self.relu(self.HiddenLayersOut[i][1]\
...
class VAE():
    ...
    def fit(self, X, y = None):
        ...
        self.model = Autoencoder(self.D_in, H = self.HiddenLayers_dim, latent_dim = self.D_out).to(device)
    ...
    def __train(self):
        self.model, self.train_losses  = train(self.epochs, self.trainloader, self.model, self.optimizer, self.loss_mse)
...
# #df_x2D = pd.concat([df['filename'], df_x2D, df[list(df_conf.columns[1:])]], axis=1)
