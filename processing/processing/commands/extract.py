...
def preview(file, start, duration, no_ffmpeg):
    ...
    start_sec = cfg.variables.get('preview_file_start',
                                  '0') if start is None else start
    dur_sec = cfg.variables.get('preview_file_dur',
                                '10') if duration is None else duration
    ...
    if not no_ffmpeg:
        print(
            '... generating wav extracts, use --no-ffmpeg to skip in case of error')
...
def all(force, skip_existing):
    ...
    for act in ["count", "do"]:
        for esr, band, spec, fname, info, input_path, output_path in iterate_audio_files_with_bands(
            ...
            if output_path.exists() and not force:
                if skip_existing:
                    ...
                    continue
                raise Exception(
                    f'"{output_path}" exists (-s to skip existing, or -f to overwrite).')
            if act == "count":
                todo += 1
            else:
                done += 1
...
def show_features_size():
    cfg = get_config()
    for esr, band, spec, fname, info, input_path, feat_path in iterate_audio_files_with_bands(
            cfg, ['@feature_base', '.pklz']):
