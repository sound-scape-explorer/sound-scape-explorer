    ...
    def calculate(self):
        ...
        for cluster in self._clusters:
            plus_limits.append(
                np.nanpercentile(
                    self._dataframe[
                        self._dataframe.index == cluster].to_numpy(),
                    95,
                    axis=0
                )
            )
        ...
        for i, cluster in enumerate(self._clusters):
            dataframe_a = self._dataframe[
                self._dataframe.index == cluster].to_numpy()

            mask_a = (dataframe_a > minus_limits[i]) * \
                     (dataframe_a < plus_limits[i])
        ...
        overlap: List[List[float]] = (dataframe_b + dataframe_b.T) / 2
