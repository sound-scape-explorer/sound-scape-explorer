...
export class ScatterPlot {
  ...
  private orbitAnimationOnNextCameraCreation = false;
  ...
  getCameraTarget(): Point3D {
    const currTarget = this.orbitCameraControls.target;
  ...
  private getPointIndicesFromBoundingBoxPickingTexture(
    ...
    const pointIndicesSelection = new Uint8Array(
  ...
  private selectBoundingBox(boundingBox: ScatterBoundingBox) {
    const pointIndices = this.getPointIndicesFromBoundingBox(boundingBox);
