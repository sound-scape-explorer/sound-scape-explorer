...
export function useIntegrationValidation() {
  ...
  const isDurationValid = useCallback(
    (integration: ConfigIntegration) => {
      ...
      const isInt = Math.floor(integration.duration) === integration.duration;

      if (!isInt) {
        return false;
      }

      const durations = collectValues(integrations, integration, 'duration');

      // noinspection RedundantIfStatementJS
      if (durations.includes(integration.duration)) {
        return false;
      }
