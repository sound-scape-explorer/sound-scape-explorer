...
def coverings(cfg, plot, show):
    ...
    for inte in integrations:
        ...
        for band in cfg.bands.keys():
            ...
            for r_name in cfg.ranges.keys():
                ...
                for s in sites:
                    ...
                    for s2 in [s2 for s2 in sites if s2 != s]:
                        print('... ... ... ... ... SITE2', s2)
                        range_times2, range_features2 = load_features_for(cfg, band, r, s2)
                        range_bins2, group_starts2 = timegroup_loaded_features(range_times2, r, inte)
                        # gather data from s2 in a time-indexed dict
                        s2_features = {}
                        for g_start,g_end,t_start,g_start_i in iterate_timegroups(r, inte, range_bins2, group_starts2):
                        ...
                        for g_start,g_end,t_start,g_start_i in iterate_timegroups(r, inte, range_bins, group_starts):
                            if t_start not in s2_features: continue
                            d_times.append(t_start)
                            feats = range_features[g_start:g_end,:]
                            d_distofmeans.append(float(np.sum(np.abs(np.mean(feats, axis=0) - np.mean(s2_features[t_start], axis=0)))))
            ...
            out_path = pathlib.Path(cfg.variables['generated_base']).joinpath('pairwise', 'covering', str(inte), band+'.json')
            ...
            if plot:
                ...
                for kplot in [k for k in list(infos['data'].values())[0] if k != 't']:
                    ...
                    plt.legend()
                    plt.title(f'Covering [{kplot}] b={band}, {inte}sec integration')
...
def volumes(cfg, plot, show):
    ...
    for inte in integrations:
        ...
        for band in cfg.bands.keys():
            ...
            if plot:
                import matplotlib.pyplot as plt # import here to have matplotlib optional
                for kplot in [k for k in list(infos['data'].values())[0] if k != 't' and k != 'i']:
                    #import seaborn as sns
                    for k,data in infos['data'].items():
                        plt.plot_date([t/3600/24 for t in data['t']], data[kplot], label=k, linestyle='-', markersize=2, alpha=.9)
                    plt.legend()
